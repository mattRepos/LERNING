...
# метод sort() может отсортировать только для изменяемых типов данных, а функция sorted(arg) исправляется ситуацию
# sorted принимает в себя любой итерируемый объект
# метод .sort() меняет строку, а sorted() ниче не меняет, sorted() возвращает новый список
...
# аргумент key в функции sorted() и метода .sort() нужны для задачи кастомной сортировки, т.е по какому то условию
# здесь в примере используется сортировка по кратности 2-и

# функции isistance(b, int) и функция type(b) == int ИЛИ type(b) is int проверяет тип переменных, но только type
# делает это более жестко(имеется ввиду что строго)
